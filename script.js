...
function calculateCreditHistoryRecovery() {
    ...
    if (loanDateInput === "" || majorDelayClosureDateInput === "") {
        document.getElementById("creditHistoryRecovery").value = ""; // Очищаем поле, если одна из дат не указана
        return;
    }
    ...
    monthsDifference += majorDelayClosureDate.getMonth();

    // Если разница меньше 1 месяца, то выводим "Восстановление не требуется"
...
document.getElementById("majorDelayClosureDate").addEventListener("input", function() {
    calculateCreditHistoryRecovery();
});
