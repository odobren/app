...
document.getElementById("loanForm").addEventListener("submit", function(event) {
  ...
  var closureDateInput = document.getElementById("closureDate").value.trim(); // Новое поле даты закрытия

  if (loanAmountInput === "" || borrowerAgeInput === "" || borrowerNameInput === "" || loanDateInput === "" || pensionContributionsInput === "" || closureDateInput === "") {
  ...
  var closureDate = new Date(closureDateInput); // Создаем объект даты закрытия
  ...
  try {
    ...
    document.getElementById("approvalAmount").value = ((pensionContributions * 8.1 / 6 / 2) / 0.0165).toFixed(2); // Сумма одобрения (не тронуто)
    document.getElementById("recoveryTerm").value = recoveryTerm; // Срок восстановления
...
// Обработчик изменения возраста заемщика для автоматического обновления срока кредита (не тронуто)
...
// Обработчик изменения поля с пенсионными отчислениями для автоматического обновления суммы одобрения (не тронуто)
...
// Функция для расчета суммы одобрения (не тронуто)
