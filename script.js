...
document.getElementById("loanForm").addEventListener("submit", function(event) {
    ...
    var latePaymentClosureDate = new Date(latePaymentClosureDateInput); // Преобразуем в объект Date
...
function calculateCreditHistoryRecoveryPeriod() {
    ...
    var latePaymentClosureDateInput = new Date(document.getElementById("latePaymentClosureDate").value);

    if (!loanDateInput || !latePaymentClosureDateInput) return; // Если даты не указаны, выходим из функции

    // Вычисляем разницу в месяцах между Датой и Датой закрытия крупной просрочки
    ...
    if (monthsDifference < 1) {
        ...
        document.getElementById("creditHistoryRecoveryPeriod").value = monthsDifference + " месяцев";
    }
}

// Обработчик изменения поля "Дата закрытия крупной просрочки" для автоматического вычисления срока восстановления кредитной истории
document.getElementById("latePaymentClosureDate").addEventListener("change", function() {
    calculateCreditHistoryRecoveryPeriod(); // Вызываем функцию для вычисления срока восстановления кредитной истории
});

// Обработчик изменения поля "Дата" для автоматического вычисления срока восстановления кредитной истории
